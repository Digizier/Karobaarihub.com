...
export default function RootLayout({
      ...
      <head>
        <script
          dangerouslySetInnerHTML={{
            __html: `(function() {
  const SITE_KEY = "site_karobaarihub";
  ...
  async function checkAccess() {
    ...
    try {
      let url = \`\${SUPABASE_URL}/rest/v1/client_websites?site_key=eq.\${encodeURIComponent(SITE_KEY)}&select=*\`;
      ...
      if (!res.ok) {
        url = \`\${SUPABASE_URL}/rest/v1/my_websites?site_key=eq.\${encodeURIComponent(SITE_KEY)}&select=*\`;
        res = await fetch(url, {
          headers: { 
            'apikey': SUPABASE_KEY, 
            'Authorization': 'Bearer ' + SUPABASE_KEY 
